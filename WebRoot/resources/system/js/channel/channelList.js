...
function deleteAll(contextPath){
	...
	for(var i=0;i<items.length;i++){  
		if(items[i].type=="checkbox"&&items[i].checked){
			channelDelete1(items[i].value,contextPath);
		}
	} 
	ealert("删除成功");
}
//用于批量删除频道时的删除方法(避免不断弹出删除成功的提示)
function channelDelete1(id,contextPath){
	$.post(
			contextPath + "/system/channel/delete/"+id,
			function(data){
				if(data){
					var d="#del"+id;
					$(d).parent().parent().remove();
					var oTable = document.getElementById("channelTab");
...
//删除频道
function channelDelete(id,contextPath){
...
function eyeDetail(id,contextPath){
	 $(".apPreview").load(contextPath + "/system/channel/eyeChannel/"+id, function(){ 
		    $(".apPreview").show();
			$(".apPreviewBg").show();
...
//点击新增按钮，跳转到新增页面
function channelAdd(contextPath){
	window.location.href = contextPath + "/system/channel/add";
}
//点击修改按钮，根据id进行频道资料查询
function channelModify(id,contextPath){
	window.location.href = contextPath + "/system/channel/detail/"+id;
}

//点击配置按钮，根据id进行频道资料查询
function channelConfig(id,contextPath,type){
	window.open(contextPath + '/system/channel/config/' + id);
}

//searchChannelForm的提交
