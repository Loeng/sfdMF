...
function userDelete1(id,contextPath){
	$.post(
		contextPath + "/system/warehouse/delete/"+id,
	 	function(data){
	 		if(data){
	 		var d="#del"+id;
...
function userAdd(contextPath){
	window.location.href = contextPath + "/system/warehouse/warehouseAdd";
}

//点击修改按钮，根据id进行会员资料查询
function userModify(id,contextPath){
	window.location.href = contextPath + "/system/warehouse/detail/"+id+"/"+1;
}

//点击修改按钮，根据id进行会员资料查询
function queryId(id,contextPath){
	window.location.href = contextPath + "/system/warehouse/detail/"+id+"/"+2;
}
