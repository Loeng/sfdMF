...
function configure(shopAdid,webroot,type){
	var currentPage = $("#currentPage").val();
	if(type=="未配置"){
		$.post(
				webroot + "/system/wlbad/wlbConfigureAd/"+shopAdid,
			 	function(data){
			 		if(data){
						 if(data=="1"){
							 ealert("配置成功");
	...
	if(type=="已配置"){
		$.post(
				webroot + "/system/wlbad/wlbCancelConfigureAd/"+shopAdid,
			 	function(data){
			 		if(data){
						 if(data=="1"){
							 ealert("取消配置成功");
