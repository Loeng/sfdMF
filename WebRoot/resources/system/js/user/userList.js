...
function userDelete1(id,contextPath){
	$.post(
		contextPath + "/system/user/delete/"+id,
	 	function(data){
	 		if(data){
	 		var d="#del"+id;
...
function userAdd(contextPath){
	window.location.href = contextPath + "/system/user/add";
}

//点击修改按钮，根据id进行会员资料查询
function userModify(id,contextPath){
	window.location.href = contextPath + "/system/user/detail/"+id;
}
...
function queryId(id,contextPath){
	$('#div1').load($("#ctxpath").val()+"/system/user/query/"+id,function(){
