...
function formSubmit(){
	//如果通过则提交 不同则显示未通过的项目
	if( checkNameBlur() && checkPasswordBlur() && checkPwdSameBlur() && checkStoreNameBlur()&&checkMobileBlur()&&checkEmailBlur()&&checkCardNumberBlur()&&checkRole()){
		document.getElementById("addStoreForm").submit();
	}else{
		checkNameBlur();
		checkPasswordBlur();
		checkPwdSameBlur();
