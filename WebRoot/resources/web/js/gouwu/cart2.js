$(document).ready(function() {
	...
	var quantity = getQueryString("quantity");
	$("#buyCount").val(parseInt(quantity));
...
if(quantity==null||quantity==""){
	...
	$("#inMain").load($("#ctxpath").val()+"/web/gouWu/loadOrder/" + pId + "/" + quantity,function() {
...
function checkNameBlur(){
	...
	if($.trim(name) == ""){
		...
		return false;
	}else if($.trim(name).length<2 || $.trim(name).length>20){
		$("#nameMsgSpan").attr("class","formError");
		$("#nameMsgSpan").css("display","inline");
...
function checkAddressBlur(){
	...
	var city = $.trim($("#tempInfoAddress").html());
	if(city==""){
...
function formSubmitUpdate(){
	if(checkNameBlur2() && checkAddressBlur2() && checkMobileBlurTempNew2()){
		var zipcodeTwo = $.trim($("#zipcode2").val());
		if(zipcodeTwo!=""){
...
//加商品数量
function addproductQuantity(contextPath,productId,oldQuantity){
	var count=$.trim($("#buyCount").val());
	count = parseInt(count) + 1;
	$("#inMain").load(contextPath + "/purchase/shopCart/checkQuantitySub/"+ productId +"/"+ count + "/" + oldQuantity);
}

//减商品数量
function jianPrice(contextPath,productId,oldQuantity){
	var count=$.trim($("#buyCount").val());
	count = parseInt(count) -1;
	$("#inMain").load(contextPath + "/purchase/shopCart/checkQuantitySub/"+ productId +"/"+ count + "/" + oldQuantity);
}
...
//直接输入商品数量
function numberCheck(contextPath,productId,oldQuantity){
	var count=$.trim($("#buyCount").val());
...
function checkLength(gm) {
	 ...
	 document.getElementById("lengthMsg").innerHTML = "还可输入"+curr.toString()+"字";
}
