// 返回列表的方法
function goBack(){
	window.location.href= $("#ctxpath").val() + "/store/order/gylList/" + $("#actionType").val() + "/" + $("#orderType").val();
}
...
function newdeliveryUt(ctxpath,orderId,logisticsName,logisticsNo,actualShipping){
	$.post(ctxpath+"/store/order/deliveryStoreOrder",{orderId:orderId, logisticsName:logisticsName, logisticsNo:logisticsNo, actualShipping:actualShipping},function(deliveryOk){
		if(deliveryOk){
			if($("#orderType").val()==2){
				choseMenu($("#ctxpath").val(),'ORDER_SALE_LSSC');
			}
			if($("#orderType").val()==0){
				choseMenu($("#ctxpath").val(),'ORDER_SALE_CP');
			}
		}else{
			falert("发货失败，稍后重试。");
		}
