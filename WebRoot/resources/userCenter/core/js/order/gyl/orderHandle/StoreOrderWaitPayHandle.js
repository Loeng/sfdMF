...
function updateSubmitPrice(contextPath,orderId){
	...
	if(judg!=1){
		econfirm("确定要修改价格吗?",updateSubmitPriceNews,[contextPath, orderId, newProductPriceTotal, newFaress],null,null);
	}
}
...
function goList(contextPath){
	window.location.href=contextPath+"/store/order/gylList/"+$("#actType").val()+"/"+$("#orderType").val();
}
// 修改金额块的显示与隐藏切换效果
function qiehuan(){
	var qh = $("#qh").val();
