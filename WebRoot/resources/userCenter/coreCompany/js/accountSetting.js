	...
	if(emailcheck()){
		$.post($("#ctxpath").val() + "/store/account/checkEmail",{email:$("#oldEmail").val(),newEmail:$("#newEmail").val()},function(data){	
		   if(data == '0'){
			   ...
			   $(".alterBox").hide();
			  // $(".alertLayerBg").hide();
			   salertWithFunction("原邮箱错误,请重新输入!",back, null);
		   }else if(data == '2'){
				   falert("修改邮箱失败,请重新修改!");
		   }else{
			   $("#oldEmail").val("");
			   ...
			   $(".alterBox").hide();
			//   $(".alertLayerBg").hide();
			   salertWithFunction("修改成功,请到邮箱激活!",back, null);
		   }
		});
	}
}
	...
	if(password()){
		$.post($("#ctxpath").val() + "/store/account/checkpassword",{password:$("#oldPwd").val(),newPassword:$("#newPwd").val()},function(data){	
			   if(data == '0'){
				   ...
				   $(".alterBox").hide();
//				   $(".alertLayerBg").hide();
				   salertWithFunction("修改成功!",back,null);
			   }
			});
	}
}
...
$.post(ctx+"/store/account/phone",
        ...
        function(data){
			  if(data=='0'){
				   salertWithFunction('修改手机号码成功',realod,null);
				   $("#oldphone").val("");
				   $("#newphone").val("");   
				   $("#yanzheng").val("");
...
function realod(){
	 window.location.href= $("#ctxpath").val() + "/store/accountmanager/list";
}
function phone(ctx){
	var reg=/^0?(13[0-9]|15[012356789]|18[0236789]|14[57])[0-9]{8}$/;
	old = $("#oldPhone").val();
