...
function checkmobileBlur(){
	...
	if(mobile == "" && phone == ""){
		$(phone_obj).css("display", "block");
		$(phone_obj).html("手机号码和电话至少填写一项！");
		
		$(mobile_obj).css("display", "block");
		$(mobile_obj).html("手机号码和电话至少填写一项！");
		return false;
	}
	if(mobile != "" && phone == ""){
		if(!regexp.test(mobile)){
			$(mobile_obj).css("display", "block");
			$(mobile_obj).html("输入有误");
			return false;
		}
	}
	if(mobile == "" && phone != ""){
