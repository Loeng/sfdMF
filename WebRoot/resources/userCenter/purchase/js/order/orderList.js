...
function deleteAll(){
	...
	if(id == ""){
		...
		econfirm('是否确认取消？',deleteOrder,[$("#ctxpath").val(),id],null,null);
	}
}
// 搜索表单提交
function formSubmit(){
...
function queRenShouHuo(ctxpath,orderId){
	window.location.href = ctxpath + "/store/Order/confirm/"+orderId;
}
//分页跳转
...
function deleteOrder(contextPath,id){
	window.location.href = contextPath + "/store/caigou/orderDelete/"+id;
}