	...
	if($("#card").val()==''){
		...
		$("#addBankCardForm").ajaxSubmit(
				...
				if( data=="1"){
					...
					econfirm('添加失败,该银行您已绑定过银行卡？',null,null,goBack,[$("#ctxpath").val()]);
				}
			});
	}
}
...
function goAdd(ctxPath){
	window.location.href=ctxPath+"/store/account/bank/add";
}
//返回列表
function goBack(contextPath){
	// 判定contextPath是否为空
	if(contextPath==null||contextPath==""){
		window.location.href=contextPath+"/store/manager/ACCOUNT_BANK_LIST";
	}
	window.location.href=contextPath + "/store/manager/ACCOUNT_BANK_LIST";
}