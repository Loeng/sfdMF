...
function productDelete1(id,contextPath){
	$.post(
		contextPath + "/store/product/delete/"+id,
		function(data){
			if(data){
				var d="#del"+id;
...
function productModify(id,contextPath,type){
	...
	window.location.href= contextPath + "/store/product/edit/"+id + "/" + type + "/" + productType;
}
...
function addProduct(){
	...
	window.location.href= contextPath + "/store/product/storeProductAddClassify?jugg="+type;
}
// 重置
function resetForm(){
